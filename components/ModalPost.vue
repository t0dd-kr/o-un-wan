<script setup lang="ts">
  const { showSortModal, sortBy, sortByOptions } = storeToRefs(usePost())
  ...
  >
    ...
    >
      ...
      >
        ...
        <div class="absolute bottom-0 left-0 flex w-full justify-center py-2">
          <span class="badge badge-neutral select-none">
            {{ post.exerciseType }}
          </span>
        </div>
      </div>
      ...
      <div class="flex flex-col overflow-auto p-2">
        <div
          class="border-base-300 flex flex-col border-b px-2 py-2"
      ...
      <div class="join px-3 py-4">
        <input
          ...
          type="text"
          class="input input-bordered bg-base-200 join-item flex-1"
        />
        <button class="btn btn-primary join-item">댓글 작성</button>
      </div>
    </div>
  </div>
</template>
